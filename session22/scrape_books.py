...
def extract_books(html):
    ...
    for li in booklist.find_all("li"):
        ...
        availability = book.find("p", attrs={"class": "availability"}).text.strip()
        print(availability)

        # Try to find the rating information of each book
        stars = book.find("p", attrs={"class": "star-rating"})["class"][1]
        print(stars)
        # One -> 1, Two -> 2
